...
function railVerifier(n, arr) {
  ...
  while(currCoach <= n) {
    ...
    while(!station.length !== 0 && station[station.length-1] === arr[arrCount]) {
      station.pop()
      arrCount++
      if(arrCount>=n) break
    }
...
console.log(railVerifier(5, [1,2,3,4,5]))
console.log(railVerifier(5, [5,4,1,2,3]))
console.log(railVerifier(6, [6,5,4,3,2,1]))